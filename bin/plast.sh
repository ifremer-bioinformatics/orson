...
# Run PLAST
if [ "$Q_TYPE" = "n" ]; then
    CMD="$PLAST_PATH -p plastx -i $QUERY -d $DB -o $CHUNK_OUTPUT_NAME -max-hit-per-query 20 -max-hsp-per-hit 1 -e 1e-3 -a $NCPUS"
    echo $CMD > $LOGCMD
    eval $CMD
else
    CMD="$PLAST_PATH -p plastp -i $QUERY -d $DB -o $CHUNK_OUTPUT_NAME -max-hit-per-query 20 -max-hsp-per-hit 1 -e 1e-3 -a $NCPUS -outfmt 4"
    echo $CMD > $LOGCMD
    eval $CMD
fi
